...
router.delete('/artists/:artist_id', authMiddleware, requireRole('admin'), async (req, res) => {
    ...
    try {
        ...
        if (songRows.length > 0) {
            ...
            await conn.query(`DELETE FROM user_likes WHERE song_id IN (${placeholders})`, songIds).catch(() => {});
            await conn.query(`DELETE FROM playlist_songs WHERE song_id IN (${placeholders})`, songIds).catch(() => {});
            await conn.query(`DELETE FROM album_tracks WHERE song_id IN (${placeholders})`, songIds).catch(() => {});
